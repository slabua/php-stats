<?php   ////////////////////////////////////////////////
        //   ___ _  _ ___     ___ _____ _ _____ ___   //
        //  | _ \ || | _ \___/ __|_   _/_\_   _/ __|  //
        //  |  _/ __ |  _/___\__ \ | |/ _ \| | \__ \  //
        //  |_| |_||_|_|0.1.8|___/ |_/_/ \_\_| |___/  //
        //                                            //
      ///////////////////////////////////////////////////
      //   Author: Roberto Valsania (Webmaster76)      //
      //    Staff: Matrix, Viewsource, PaoDJ, Fabry    //
      // Homepage: www.php-stats.com,                  //
      //           www.php-stats.it,                   //
	  //           www.php-stat.com                    //              
      ///////////////////////////////////////////////////


                 if(!isset($_GET)) $_GET=$HTTP_GET_VARS;
            if(isset($_GET['ip'])) $ip=addslashes($_GET['ip']);     else $ip="";
          if(isset($_GET['page'])) $page=addslashes($_GET['page']); else $page="";
          if(isset($_GET['what'])) $what=addslashes($_GET['what']); else $what="";
              if(!isset($_COOKIE)) $_COOKIE=$HTTP_COOKIE_VARS;
if(isset($_COOKIE['pass_cookie'])) $pass_cookie=$_COOKIE['pass_cookie']; else $pass_cookie="";
$style=""; // In caso di register globals=on
define('IN_PHPSTATS',true);

// inclusione delle principali funzioni esterne
if(!@include("config.php")) die("<b>ERRORE</b>: File config.php non accessibile.");
if(!@include("inc/main_func.inc.php")) die("<b>ERRORE</b>: File main_func.inc.php non accessibile.");
if(!@include("inc/admin_func.inc.php")) die("<b>ERRORE</b>: File admin_func.inc.php non accessibile.");
//
$start_time=get_time();
if($option['out_compress']) ob_start("ob_gzhandler");
if($option['prefix']=="") $option['prefix']="php_stats";
// Connessione a MySQL e selezione database
db_connect();
// Lettura delle variabili
$result=sql_query("SELECT name,value FROM $option[prefix]_config");
while($row=@mysql_fetch_array($result)) $option[$row[0]]=$row[1];

if($option['use_pass']) if($pass_cookie!=md5($option['admin_pass'])) { header("Location: $option[script_url]/admin.php?action=login"); die(); }  
if($option['template']=="") $option['template']="default";
$template_path="templates/".$option['template'];

include("lang/".$option['language']."/main_lang.inc");
include("$template_path/def.php");

$page=str_replace("§§","&",$page);
$trckpage ="\n<html>\n<head>\n<title>$tracking[title]</title>";
$trckpage.="\n<link rel=\"stylesheet\" href=\"$template_path/styles.css\" type=\"text/css\">";
$trckpage.="\n<META NAME=\"ROBOTS\" CONTENT=\"NONE\">";
$trckpage.="\n<script>";
$trckpage.="\nfunction whois(url) {";
$trckpage.="\n\ttest=window.open(url,'whois','SCROLLBARS=1,STATUS=NO,TOOLBAR=NO,RESIZABLE=YES,LOCATION=NO,MENU=NO,WIDTH=450,HEIGHT=600,LEFT=0,TOP=0');";
$trckpage.="\n\t}";
$trckpage.="\n</script>";
$trckpage.="\n<script language=\"javascript\" src=\"$template_path/functions.js\"></script>";
$trckpage.="\n</head>";
$trckpage.="\n\n<body bgcolor=\"$style[bg_pops]\" onload=\"self.focus()\">\n";
switch ($what)
  {
  case 'referer':
    $trckpage.="<span class=\"testo\"><center>".str_replace("%URL%",formaturl($page,"",40,10,-30),$tracking['referer'])."</center><br>";
    $result=sql_query("SELECT ip,date,referer FROM $option[prefix]_details WHERE referer='$page' ORDER BY date DESC");
    if(@mysql_num_rows($result)>0)
      {
      $total=0;
      $trckpage.="<table $style[table_header] width=\"400\">";
      $trckpage.="<table $style[table_header] width=\"260\">";
      $trckpage.="<tr bgcolor=\"$style[table_title_bgcolor]\">";
	  $trckpage.="<td width=\"70\" bgcolor=\"$style[table_title_bgcolor]\"><span class=\"tabletitle\"><center>".$tracking['referer_date']."</center></span></td>";
	  $trckpage.="<td width=\"70\" bgcolor=\"$style[table_title_bgcolor]\"><span class=\"tabletitle\"><center>".$tracking['referer_time']."</center></span></td>";
	  $trckpage.="<td width=\"120\" bgcolor=\"$style[table_title_bgcolor]\"><span class=\"tabletitle\"><center>".$tracking['referer_ip']."</center></span></td>";
	  $trckpage.="</tr>";
      $trckpage.="</table>";
      while($row=@mysql_fetch_array($result))
        {
        $total++;
        $trckpage.="<table $style[table_header] width=\"260\">";
        $trckpage.="<tr bgcolor=\"#B3C0D7\" onmouseover=\"setPointer(this, '#CCCCCC', '$style[table_bgcolor]')\" onmouseout=\"setPointer(this, '$style[table_bgcolor]', '$style[table_bgcolor]')\">";
		$trckpage.="<td width=\"70\" bgcolor=\"$style[table_bgcolor]\"><span class=\"tabletextA\">".formatdate($row[1],3)."</span></td>";
		$trckpage.="<td width=\"70\" bgcolor=\"$style[table_bgcolor]\"><span class=\"tabletextA\">".formattime($row[1])."</span></td>";
		$trckpage.="<td width=\"120\" bgcolor=\"$style[table_bgcolor]\"><span class=\"tabletextA\">";
    	if($option['ext_whois']=="") $trckpage.="<a href=\"javascript:whois('whois.php?IP=$row[0]');\">$row[0]</a>";
	                            else $trckpage.="<a href=\"".str_replace("%IP%",$row[0],$option['ext_whois'])."\" target=\"_BLANK\">$row[0]</a>";
		$trckpage.="</span></td>";
		$trckpage.="</tr>";
        $trckpage.="</table>";
        }
      $trckpage.="<center><br>".str_replace("%total%",$total,$tracking['total'])."<br><br>$tracking[fine]</center>";
      }
      else
      {
      $trckpage.="<center><span class=\"tabletextA\"><br>$tracking[noresult]<br><br>$tracking[fine]<br></span></center>";
      }
  break;

    case 'ip':
    $result=sql_query("SELECT ip,date,currentPage FROM $option[prefix]_details WHERE ip='$page' ORDER BY date ASC");
    if(@mysql_num_rows($result)>0)
      {
      $total=0;
      $trckpage.="<table $style[table_header] width=\"600\">";
      $trckpage.="<tr>";
	  $trckpage.="<td width=\"120\" bgcolor=\"$style[table_title_bgcolor]\"><span class=\"tabletitle\"><center>Data</center></span></td>";
	  $trckpage.="<td width=\"440\" bgcolor=\"$style[table_title_bgcolor]\"><span class=\"tabletitle\"><center>Pagina visitata</center></span></td>";
	  $trckpage.="</tr>";
      $trckpage.="</table>";
      while($row=@mysql_fetch_array($result))
        {
        $total++;
        $trckpage.="<table $style[table_header] width=\"600\">";
        $trckpage.="<tr bgcolor=\"#B3C0D7\" onmouseover=\"setPointer(this, '#CCCCCC', '$style[table_bgcolor]')\" onmouseout=\"setPointer(this, '$style[table_bgcolor]', '$style[table_bgcolor]')\">";
		$trckpage.="<td width=\"120\" bgcolor=\"$style[table_bgcolor]\"><span class=\"tabletextA\">".formatdate($row[1],3)." - ".formattime($row[1])."</span></td>";
		$trckpage.="<td width=\"440\" bgcolor=\"$style[table_bgcolor]\"><span class=\"tabletextA\">".formaturl($row[2], "", 70, 30, -35)."</span></td>";
		$trckpage.="</tr>";
        $trckpage.="</table>";
        }
      $trckpage.="<center><span class=\"tabletextA\"><br>".str_replace("%total%",$total,$tracking['total'])."<br><br>$tracking[fine]</span></center>";
      }
      else
      {
      $trckpage.="<center><span class=\"tabletextA\">$tracking[noresult]<br><br>$tracking[fine]</span></center>";
      }
  break;

	///////////////////////////////////////      
	// PAGINE VISTE DAGLI UTENTI ON LINE //
	///////////////////////////////////////
	case 'online':
	$total=0;
	if($option['ext_whois']=="") $tmp="<a href=\"javascript:whois('whois.php?IP=$ip');\">$ip</a>";
	                        else $tmp="<a href=\"".str_replace("%IP%",$ip,$option['ext_whois'])."\" target=\"_BLANK\">$ip</a>";
      $trckpage.="<span class=\"testo\"><center>".str_replace("%IP%",$tmp,$tracking['online'])."</center><br>";
	  $result=sql_query("SELECT visitor_id FROM $option[prefix]_cache WHERE user_id='$ip'");
	  if(mysql_num_rows($result)>0)
	    {
        $trckpage.="<table $style[table_header] width=\"530\">";
        $trckpage.="<tr bgcolor=\"$style[table_title_bgcolor]\">";
	    $trckpage.="<td width=\"60\" bgcolor=\"$style[table_title_bgcolor]\"><span class=\"tabletitle\"><center>".$tracking['online_time']."</center></span></td>";
	    $trckpage.="<td width=\"470\" bgcolor=\"$style[table_title_bgcolor]\"><span class=\"tabletitle\"><center>".$tracking['online_url']."</center></span></td>";
        $trckpage.="</tr>";
        $trckpage.="</table>";	  
	    list($id)=@mysql_fetch_row($result);
	    $result=sql_query("SELECT currentPage,date,titlePage FROM $option[prefix]_details WHERE visitor_id='$id' ORDER BY 'date' ASC");
	    if(@mysql_num_rows($result)>0) 
		  {
	      while($row=@mysql_fetch_array($result))
            {
            $total++;
		    $trckpage.="<table $style[table_header] width=\"530\">";
            $trckpage.="<tr bgcolor=\"#B3C0D7\" onmouseover=\"setPointer(this, '#CCCCCC', '$style[table_bgcolor]')\" onmouseout=\"setPointer(this, '$style[table_bgcolor]', '$style[table_bgcolor]')\">";
            $trckpage.="<td width=\"60\" bgcolor=\"$style[table_bgcolor]\"><span class=\"tabletextA\">".formattime($row[1])."</span></td>"; 
            $trckpage.="<td width=\"470\" bgcolor=\"$style[table_bgcolor]\"><span class=\"tabletextA\">".formaturl($row[0],"",80,35,-40,$row[2])."</span></td>";
            $trckpage.="</tr>";
            $trckpage.="</table>";
	        }
		  $trckpage.="<center><br>".str_replace("%total%",$total,$tracking['total'])."<br><br>$tracking[fine]</center>";
		  }
		  else
		  {
		  $trckpage.="<center>".$tracking['online_err_nopage']."<br><br>$tracking[fine]</center>";
		  }
		}
		else
		{
		$trckpage.="<center><span class=\"tabletextA\"><br>".$tracking['online_err_nonoline']."<br><br>$tracking[fine]<br></span></center>";
		}
	  break;
	  
	///////////////////////////////////////
	// TRACKING DEI REFERERS PER DOMINIO //
	///////////////////////////////////////
	case 'referer_domain':
	if(isset($_GET['domain'])) $domain=addslashes($_GET['domain']); else $domain="";
	  if(isset($_GET['mese'])) $mese=addslashes($_GET['mese']); else $mese="";
	if($mese=="")  
	  $result=sql_query("SELECT * FROM $option[prefix]_referer WHERE data LIKE '$domain%' ORDER BY visits DESC");
	  else
	  $result=sql_query("SELECT * FROM $option[prefix]_referer WHERE data LIKE '$domain%' AND mese='$mese' ORDER BY visits DESC");
	if(@mysql_num_rows($result)>0)
	  {
      $trckpage.="<span class=\"testo\"><center>".str_replace("%DOMAIN%",$domain,$tracking['domain_title'])."</center><br>";
	  if($mese!="") 
	    {
		list($anno,$mese)=explode("-",$mese);
		$mese=$varie['mounts'][$mese-1];
		$tmp=$tracking['domain_mese'];
		$tmp=str_replace("%MESE%",$mese,$tmp);
		$tmp=str_replace("%ANNO%",$anno,$tmp);
		$trckpage.="<span class=\"testo\"><center>$tmp</center></span><br>";
		}
      $total=0;
      $trckpage.="<table $style[table_header] width=\"530\">";
      $trckpage.="<tr bgcolor=\"$style[table_title_bgcolor]\">";
	  $trckpage.="<td width=\"500\" bgcolor=\"$style[table_title_bgcolor]\"><span class=\"tabletitle\"><center>".$tracking['domain_url']."</center></span></td>";
	  $trckpage.="<td width=\"30\" bgcolor=\"$style[table_title_bgcolor]\"><span class=\"tabletitle\"><center>".$tracking['domain_hits']."</center></span></td>";
	  $trckpage.="</tr>";
      $trckpage.="</table>";
	  while($row=@mysql_fetch_array($result))  
        {
        $total++;
        $trckpage.="<table $style[table_header] width=\"530\">";
        $trckpage.="<tr bgcolor=\"#B3C0D7\" onmouseover=\"setPointer(this, '#CCCCCC', '$style[table_bgcolor]')\" onmouseout=\"setPointer(this, '$style[table_bgcolor]', '$style[table_bgcolor]')\">";
        $trckpage.="<td width=\"500\" bgcolor=\"$style[table_bgcolor]\"><span class=\"tabletextA\">".formaturl($row[0], "", 80, 35, -40)."</span></td>";
		$trckpage.="<td width=\"30\" bgcolor=\"$style[table_bgcolor]\" align=\"right\"><span class=\"tabletextA\">$row[1]</span></td>"; 
        $trckpage.="</tr>";
        $trckpage.="</table>";
	    }
	  $trckpage.="<center><br>".str_replace("%total%",$total,$tracking['total'])."<br><br>$tracking[fine]</center>";
	  }
	  else
	  {
	  $trckpage.="<center><span class=\"tabletextA\"><br>".$tracking['domain_err_nopage']."<br><br>$tracking[fine]<br></span></center>";
	  }
	break;
	  
  default:
    // V1.1
	$result=sql_query("SELECT ip,date,currentPage FROM $option[prefix]_details WHERE currentPage='$page' ORDER BY date DESC");
    if(@mysql_num_rows($result)>0)
      {
      $trckpage.="<span class=\"testo\"><center>".str_replace("%url%",formaturl($page,"",40,10,-30),$tracking['pages'])."</center><br>";
      $total=0;
      $trckpage.="<table $style[table_header] width=\"260\">";
      $trckpage.="<tr bgcolor=\"$style[table_title_bgcolor]\">";
	  $trckpage.="<td width=\"70\" bgcolor=\"$style[table_title_bgcolor]\"><span class=\"tabletitle\"><center>".$tracking['pages_date']."</center></span></td>";
	  $trckpage.="<td width=\"70\" bgcolor=\"$style[table_title_bgcolor]\"><span class=\"tabletitle\"><center>".$tracking['pages_time']."</center></span></td>";
	  $trckpage.="<td width=\"120\" bgcolor=\"$style[table_title_bgcolor]\"><span class=\"tabletitle\"><center>".$tracking['pages_ip']."</center></span></td>";
	  $trckpage.="</tr>";
      $trckpage.="</table>";
      while($row=@mysql_fetch_array($result))
        {
        $total++;
        $trckpage.="<table $style[table_header] width=\"260\">";
        $trckpage.="<tr bgcolor=\"#B3C0D7\" onmouseover=\"setPointer(this, '#CCCCCC', '$style[table_bgcolor]')\" onmouseout=\"setPointer(this, '$style[table_bgcolor]', '$style[table_bgcolor]')\">";
		$trckpage.="<td width=\"70\" bgcolor=\"$style[table_bgcolor]\"><span class=\"tabletextA\">".formatdate($row[1],3)."</span></td>";
		$trckpage.="<td width=\"70\" bgcolor=\"$style[table_bgcolor]\"><span class=\"tabletextA\">".formattime($row[1])."</span></td>";
		$trckpage.="<td width=\"120\" bgcolor=\"$style[table_bgcolor]\"><span class=\"tabletextA\">";
    	if($option['ext_whois']=="") $trckpage.="<a href=\"javascript:whois('whois.php?IP=$row[0]');\">$row[0]</a>";
	                            else $trckpage.="<a href=\"".str_replace("%IP%",$row[0],$option['ext_whois'])."\" target=\"_BLANK\">$row[0]</a>";		
		
		$trckpage.="</span></td>";
		$trckpage.="</tr>";
        $trckpage.="</table>";
        }
      $trckpage.="<center><br>".str_replace("%total%",$total,$tracking['total'])."<br><br>$tracking[fine]</center>";
      }
      else
      {
      $trckpage.="<center><span class=\"tabletextA\"><br>$tracking[noresult]<br><br>$tracking[fine]<br></span></center>";
      }
  break;
  }

//$trckpage.="<script>self.focus()</script>";
$trckpage.="<br><span class=\"testo\"><center><a href=\"javascript:window.close();\">$tracking[close_window]</a></span></center>\n\n</body>\n</html>";

echo $trckpage;

// Chiusura connessione a MySQL se necessario
if($option['persistent_conn']!=1) mysql_close();
?>
