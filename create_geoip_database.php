...
if($fp==false)
  {
  die('<br><br><b>ERRORE: il file GeoIPCountryWhois.csv non è stato trovato!!!</b>');
  }
else
  {
  mysql_query('DROP TABLE /*!32300 IF EXISTS*/ '.$option['prefix'].'_ip_zone;');
  mysql_query('CREATE TABLE '.$option['prefix']."_ip_zone (ip_from int(10) unsigned NOT NULL default 0, ip_to int(10) unsigned NOT NULL default 0, tld char(2) NOT NULL default '', PRIMARY KEY (ip_from,ip_to)) ENGINE=MyISAM;");
  ...
  while(!feof($fp))
    ...
    if($content!='')
      {
      list($dummy1,$dummy2,$ip_from,$ip_to,$tld,$dummy3)=explode(',',$content);
      $tld=strtolower($tld);
