...
switch ($mode)
  {
    ...
    $result=sql_query("SELECT data FROM $option[prefix]_cache WHERE data>$date");
    $online=@mysql_num_rows($result);
    $result=sql_query("SELECT * FROM $option[prefix]_cache WHERE user_id='$ip' AND data>$date");
    $ip_presente=@mysql_num_rows($result);
    if($ip_presente!=1) $online++;
    $toshow=$online;
    break;

    // VISITATORI DI OGGI (0.1.5)
    case '1':
	$tmp1=0;
	$tmp2=0;
	...
	// Visitatori totali
	case '3':
	list($visite)=@mysql_fetch_row(sql_query("SELECT visits FROM $option[prefix]_counters LIMIT 1"));
    $toshow=$visite+$option['startvisits'];
	break;
