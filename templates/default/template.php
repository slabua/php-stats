...
if($refresh) 
  ...
  if(!in_array($trad_action,$norefresh_action)) // Alcune pagine sono escluse dal refresh
	$meta.="\n<META HTTP-EQUIV=\"refresh\" CONTENT=\"$option[autorefresh];URL=".$option['script_url']."/admin.php"."?"."$QUERY_STRING\">";if($update_msg) $meta.="\n".$update_msg;
...
// Error-log viewer
if(!$option['logerrors'])
  $template=preg_replace("'<!--Begin errorlogviewer--[^>]*?>.*?<!--End errorlogviewer-->'si","", $template);
?>