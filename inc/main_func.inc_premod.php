...
function getbrowser($arg){
  $browser=" ".ereg_replace(" ","",$arg);
  $bw=file("def/bw.dat");
  $flag=0;
  while(list($key,$val)=each($bw))
    {
    list($nome_bw,$id_bw)=explode("|",$val);
    if(strpos($browser,chop($id_bw))) { $flag++; break;}
    }
  if($flag==0) logerrors("Unknown BW|$arg");
  return($flag==0 ? "?" : $nome_bw);
}
