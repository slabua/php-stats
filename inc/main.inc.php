...
if($modulo[6]):
  ...
  if(mysql_affected_rows()>0) {
	...
	$giorno_inizio=($row[0]); // escludi oggi e primo giorno // by -=JackNight=-
...
// GIORNO "PEGGIORE" (pagine)
$result=sql_query("SELECT data, hits FROM $option[prefix]_daily 
WHERE data != NOW() AND data != '$giorno_inizio' ORDER BY hits ASC LIMIT 1"); // escludi oggi & primo giorno // by -=JackNight=-
if(mysql_num_rows($result)>0) {
...
// GIORNO "MIGLIORE" (visite) by -=JackNight=-
...
if(mysql_num_rows($result)>0) {
  ...
  $max_visits_data=str_replace("%mount%",formatmount($data[1]),$varie['date_format']);
  $max_visits_data=str_replace("%day%",$data[2],$max_visits_data);
  $max_visits_data=str_replace("%year%",$data[0],$max_visits_data);
  }	
else { $max_visits="-"; $max_visits_data="-"; }

// GIORNO "PEGGIORE" (visite) by -=JackNight=-
$result=sql_query("SELECT data, visits FROM $option[prefix]_daily 
WHERE data != NOW() AND data != '$giorno_inizio' ORDER BY visits ASC LIMIT 1"); // escludi oggi & primo giorno // by -=JackNight=-
if(mysql_num_rows($result)>0) {
  list($min_visits_data,$min_visits)=@mysql_fetch_row($result);
  $data=explode("-",$min_visits_data);
  $min_visits_data=str_replace("%mount%",formatmount($data[1]),$varie['date_format']);
  $min_visits_data=str_replace("%day%",$data[2],$min_visits_data);
  $min_visits_data=str_replace("%year%",$data[0],$min_visits_data);
  }	
else { $min_visits="-"; $min_visits_data="-"; }

// QUERY PIU' CERCATA // by -=JackNight=-
...
else { $most_used_engine=("-"); $most_used_engine_volte=("-");}

// CALCOLO PERCENTUALE DEGLI ACCESSI DA MOTORI (dal totale escluso numero impostato in partenza) // by -=JackNight=-

$percent_engine_hits = ((100 * $total_engine)/ $visite_totali);
$percent_engine_hits = round($percent_engine_hits,1);

// TOTALE ACCESSI DA MOTORI + REFERER // by -=JackNight=-

$total_extern = ($total_referer + $total_engine);

// CALCOLO PERCENTUALE DEGLI ACCESSI DA MOTORI + REFERER (escluso numero di partenza) // by -=JackNight=-

$percent_total_extern = ((100 * $total_extern)/ $visite_totali);
...
if($modulo[6]):
  ...
  // GIORNO MIGLIORE/PEGGIORE (visite) // by -=JackNight=-
  $tmp=str_replace("%NUM%",$max_visits ,$string['max_visits']);
  $return.="\n\t<TR><TD align=right bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">$tmp</span></TD><TD bgcolor=$style[table_bgcolor]><span class=\"tabletextB\">$max_visits_data</span></TD>";  
  $tmp3=str_replace("%NUM%",$min_visits ,$string['min_visits']);
  ///////////
  $return.="\n\t<TR><TD align=right bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">$tmp3</span></TD><TD bgcolor=$style[table_bgcolor]><span class=\"tabletextB\">$min_visits_data</span></TD>";  
  $tmp1=str_replace("%NUM%",$min_hits ,$string['min_hits']);
  $return.="\n\t<TR><TD align=right bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">$tmp1</span></TD><TD bgcolor=$style[table_bgcolor]><span class=\"tabletextB\">$min_hits_data</span></TD>";
  $tmp2=str_replace("%NUM%",$max_hits ,$string['max_hits']);
  $return.="\n\t<TR><TD align=right bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">$tmp2</span></TD><TD bgcolor=$style[table_bgcolor]><span class=\"tabletextB\">$max_hits_data</span></TD>";
endif;
if($modulo[3]):
...
///////////////////////
// Motori Di Ricerca // by -=JackNight=-
///////////////////////  
  ...
  $return.="\n\t<TR><TD align=\"center\" bgcolor=$style[table_title_bgcolor] colspan=\"2\"><span class=\"tabletitle\"><center>$string[se_title]</center></span></TD></TR>";
     $return.="\n\t<TR><TD align=right bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">$string[main_tot_engine] ($percent_engine_hits%)</span></TD><TD bgcolor=$style[table_bgcolor]><span class=\"tabletextB\">$total_engine</span></TD>";
	 $return.="\n\t<TR><TD align=right bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">$string[main_tot_extern] ($percent_total_extern%)</span></TD><TD bgcolor=$style[table_bgcolor]><span class=\"tabletextB\">$total_extern</span></TD>";
	 $return.="\n\t<TR><TD align=right bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">$string[searched_word_most] ($most_query_word_volte)</span></TD><TD bgcolor=$style[table_bgcolor]><span class=\"tabletextB\">$most_query_word</span></TD>";
	 $return.="\n\t<TR><TD align=right bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">$string[main_most_engine] ($most_used_engine_volte)</span></TD><TD bgcolor=$style[table_bgcolor]><span class=\"tabletextB\">$most_used_engine</span></TD>";
...
// Utente Medio //
//////////////////
$result=sql_query("SELECT os,SUM(hits) AS dummy FROM $option[prefix]_systems GROUP BY os ORDER BY 'dummy' DESC LIMIT 1");
if(mysql_num_rows($result)==0){
	$au_os='-';
	$icon_os='';
}
else{
	list($au_os,$dummy)=mysql_fetch_row($result);
	$icon_os='<img src="images/os.php?q='.str_replace(' ','-',$au_os).'">';
}

$result=sql_query("SELECT bw,SUM(hits) AS dummy FROM $option[prefix]_systems GROUP BY bw ORDER BY 'dummy' DESC LIMIT 1");
if(mysql_num_rows($result)==0){
	$au_bw='-';
	$icon_bw='';
}
else{
	list($au_bw,$dummy)=mysql_fetch_row($result);
	$icon_bw='<img src="images/browsers.php?q='.str_replace(' ','-',$au_bw).'">';
}

$result=sql_query("SELECT reso,SUM(hits) AS dummy FROM $option[prefix]_systems GROUP BY reso ORDER BY 'dummy' DESC LIMIT 1");
if(mysql_num_rows($result)==0) $au_resolution='-';
else list($au_resolution,$dummy)=mysql_fetch_row($result);

$result=sql_query("SELECT colo,SUM(hits) AS dummy FROM $option[prefix]_systems GROUP BY colo ORDER BY 'dummy' DESC LIMIT 1");
if(mysql_num_rows($result)==0) $au_colors='-';
else{
	list($au_colors,$dummy)=mysql_fetch_row($result);
	$au_colors.=' bit';
}

$result=sql_query("SELECT tld,area,hits AS dummy FROM $option[prefix]_domains ORDER BY 'dummy' DESC LIMIT 2");
$flag=FALSE;
while(list($au_country,$au_area,$dummy)=mysql_fetch_row($result)){
	if($au_country=='unknown') continue; //scarto gli unknown
	else{
		$flag=TRUE;
		break;
	}
...
if($flag){
	...
	$au_area=substr($au_area,0,strlen($au_area)-1);
	$au_country=$domain_name[$au_country].' ('.$au_area.')';
}
else $au_country='-';

$result=sql_query("SELECT lang,hits AS dummy FROM $option[prefix]_langs ORDER BY 'dummy' DESC LIMIT 2");
$flag=FALSE;
while(list($au_lang,$dummy)=mysql_fetch_row($result)){
	if($au_lang=='unknown') continue; //scarto gli unknown
	else{
		$flag=TRUE;
		break;
	}
...
if($flag){
	...
	$au_lang=$bw_lang[$au_lang];
}
else $au_lang='-';

$return.="\n\n<!--  SHOW AVERAGE USER -->".
  "\n<br>".
  "\n<span class=\"pagetitle\"><center>$string[main_averageuser_title]</center></span>".
  "\n<table $style[table_header] width=\"95%\" border=\"0\">".
  "\n\t<TR>".
