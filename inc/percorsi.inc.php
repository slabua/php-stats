...
foreach($what as $key){
	...
	if($total>0){
		...
		while($row=mysql_fetch_array($result)){
			$page.=
			"<tr>".
			"<td align=left bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">".formaturl($row[0],$row[2],55,22,-25,$row[2],$mode)."</span></td>".
			"<td align=right bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">$row[1]</span></td>".
			"<td align=right bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">".round($row[1]/$total*100,2)."%</span></td>".
			"</tr>";
		}
