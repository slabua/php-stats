...
$return.="document.codice.downcode.value=\"<a href='$option[script_url]/click.php?id=\"+id+\"'>Click</a>\";\n";
$return.="}\n";
$return.="</SCRIPT>\n";
...
$result=sql_query("SELECT * FROM $option[prefix]_clicks ORDER BY 'clicks' DESC LIMIT $start,$rec_pag");
if(@mysql_num_rows($result)>0) {
  ...
  while($row=@mysql_fetch_array($result))
    {
    $id=$row[0];
    $clicks_log_nome=$row[1];
    $clicks_log_url=$row[2];
    $clicks_log_accessi=$row[4];
    $current++;
    ...
    $return.="<td bgcolor=$style[table_bgcolor]><span class=\"tabletextA\"><center><a href=\"javascript:link('$id');\" onClick=\"link('$id')\">$id</a></center></span></td><td bgcolor=$style[table_bgcolor]><span class=\"tabletextA\">&nbsp;<b>$clicks_log_accessi</b></span></td><td bgcolor=$style[table_bgcolor]><center>".checkfile($clicks_log_url)."</center></td></tr>";
    }
  $return.="<tr><td height=\"1\" bgcolor=$style[table_title_bgcolor] colspan=\"6\" nowrap></td></tr>";
    if($numero_pagine>1)
    {
    $return.="<tr><td bgcolor=$style[table_bgcolor] colspan=\"6\" height=\"20\" nowrap>".pag_bar("admin.php?action=clicks",$pagina_corrente,$numero_pagine,$rec_pag)."</td></tr>";
    $return.="<tr><td height=\"1\" bgcolor=$style[table_title_bgcolor] colspan=\"6\" nowrap></td></tr>";
    }
  $return.="</table>";		
  // GENERA CODICE
  ...
  $return.="<tr><td bgcolor=$style[table_title_bgcolor]><span class=\"tabletitle\">$string[click_codescript]</span></td></tr>";
  $return.="<tr><td bgcolor=$style[table_bgcolor]><center><textarea name=\"downcode\" cols=\"100\" rows=\"2\">$string[click_downcli]</textarea></center></form></td></tr>";
  $return.="</table>";
  }
  else
  {
  $return.=info_box($string['information'],$error['clicks']);
  }
return($return);
}
?>
